...
pub(crate) fn enable_fast_pwm(
    timer1: &arduino_hal::pac::TC1,
    oc1b: Pin<Input<Floating>, PB2>,
    mode: PrescaleMode,
) {
    set_wgm_15(timer1);
    set_com_3(timer1);
...
fn set_wgm_15(timer1: &arduino_hal::pac::TC1) {
    ...
    timer1
        .tccr1b
        .modify(|r, w| w.wgm1().bits(0b11).cs1().bits(r.cs1().bits()));
}
...
fn set_prescaler(timer1: &arduino_hal::pac::TC1, mode: PrescaleMode) {
    ...
    timer1
        .tccr1b
        .modify(|r, w| w.wgm1().bits(r.wgm1().bits()).cs1().bits(selected));
}
